...
public class Day3 {
    ...
    public static int closestIntersecting(Set<Pair<Integer, Integer>> s1, Set<Pair<Integer, Integer>> s2) {
        s1.retainAll(s2); // s1 becomes intersection of S1 and S2
        //noinspection OptionalGetWithoutIsPresent
        return s1.stream().map(p -> Math.abs(p.getKey()) + Math.abs(p.getValue())).min(Comparator.naturalOrder()).get();
    }

    public static Set<Pair<Integer, Integer>> getPath(String thread) {
        ...
        for (String instruction : instructions) {
            ...
            switch(direction) {
                case 'R':
                    for(i=0; i<magnitude; i++, x++) { path.add(new Pair<>(x, y)); }
                    break;
                case 'L':
                    for(i=0; i<magnitude; i++, x--) { path.add(new Pair<>(x, y)); }
                    break;
                case 'U':
                    for(i=0; i<magnitude; i++, y++) { path.add(new Pair<>(x, y)); }
                    break;
                case 'D':
                    for(i=0; i<magnitude; i++, y--) { path.add(new Pair<>(x, y)); }
                    break;
    ...
    public static void main(String[] args) {
        ...
        Set<Pair<Integer, Integer>> p1 = getPath(thread1);
        Set<Pair<Integer, Integer>> p2 = getPath(thread2);
